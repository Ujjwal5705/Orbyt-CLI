...
export const availableTools = [
    ...
    },
    {
        id: "url_context",
        name: "URL Context",
        description: "Provide specific URLs that you want the model to analyze directly from the prompt. Supports upto 20 URLs per request.",
...
export function getEnabledTools() {
    try {
        ...
        if (enabledTools.length > 0) {
            console.log(chalk.gray(`[DEBUG] Enabled tools: ${enabledTools.length}`));
            return enabledTools;
        } else {
        ...
        console.error(chalk.red('[ERROR] Failed to initialise tools:'), error.message);
        return undefined;
    }
}
...
export function resetTools(){
    ...
    console.log(chalk.gray(`[DEBUG] All tools have been set (disabled)`))
}